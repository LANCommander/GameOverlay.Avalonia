// Direct3D 8 present hooks.
//
// D3D8, like D3D9, has no DXGI, so it is hooked exactly like OpenGL and Vulkan
// are: by reading the device vtable from a throwaway probe device and patching
// the functions in place with MinHook. We hook EndScene (where the game is
// mid-scene and the backbuffer is the current target, so the overlay can be
// drawn) and Reset (to drop our D3DPOOL_DEFAULT texture before the device is
// reset).
//
// Direct3DCreate8 is resolved with GetProcAddress rather than an import, so this
// translation unit never forces d3d8.dll to load into a game that does not use
// it - the same rule the D3D9 and OpenGL hooks follow.
//
// d3d8.h was removed from the modern Windows SDK, so the headers are vendored
// under third_party/d3d8.
...
// IDirect3DDevice8 vtable slots. Fixed by COM binary compatibility, so
// hardcoding them is safe - the same values every D3D8 tool has relied on.
//   14 Reset
//   15 Present
//   35 EndScene
constexpr int kSlotReset = 14;
constexpr int kSlotEndScene = 35;
...
D3D8Renderer g_renderer8;
bool         g_installed = false;
bool         g_published = false;
thread_local bool tls_inEndScene = false;

// Reads the backbuffer size and the device window. D3D8 has no GetSwapChain
// accessor (unlike D3D9), so the size comes straight from the current
// backbuffer surface and the window from the creation parameters.
void ResolveWindowAndSize(IDirect3DDevice8* device, HWND& hwnd, uint32_t& width, uint32_t& height) {
    hwnd = nullptr;
    width = height = 0;
...
void PublishSize(IDirect3DDevice8* device, SharedState* state, bool first) {
    HWND hwnd = nullptr;
    ...
    if (first) {
        state->graphicsApi = kGraphicsApiD3D8;
        ...
        OVERLAY_LOG("D3D8 present detected (hwnd 0x%llX, %ux%u)",
...
HRESULT STDMETHODCALLTYPE EndSceneDetour(IDirect3DDevice8* device) {
    if (!tls_inEndScene) {
        tls_inEndScene = true;
        ...
        if (SharedState* state = GetSharedState()) {
            ...
            if (HostIsAlive(state)) {
                g_renderer8.Render(device, state);
...
HRESULT STDMETHODCALLTYPE ResetDetour(IDirect3DDevice8* device, D3DPRESENT_PARAMETERS* params) {
    // The DEFAULT-pool texture must be gone before Reset, or Reset fails outright.
    g_renderer8.OnDeviceLost();
...
// because the functions live in d3d8.dll, which remains loaded - but the vtable
...
bool CaptureTargets(void*& endScene, void*& reset) {
    ...
    if (!d3d8) return false;   // not a D3D8 game; nothing to hook

    auto create = reinterpret_cast<Direct3DCreate8Fn>(GetProcAddress(d3d8, "Direct3DCreate8"));
    if (!create) return false;

    IDirect3D8* d3d = create(D3D_SDK_VERSION);
    if (!d3d) {
        OVERLAY_LOG("Direct3DCreate8 returned null");
        return false;
    }
    ...
    wc.lpszClassName = L"AvaloniaOverlayD3D8Probe";
    RegisterClassExW(&wc);

    // A real, non-degenerate window: a 1x1 window has essentially no client
    // area, and deriving a backbuffer from that can make CreateDevice fail. The
    ...
    if (!hwnd) {
        d3d->Release();
        OVERLAY_LOG("D3D8 probe window creation failed: %lu", GetLastError());
        return false;
    }
    ...
    if (FAILED(hr) || !device) {
        DestroyWindow(hwnd);
        d3d->Release();
        OVERLAY_LOG("D3D8 probe device creation failed: 0x%08lX", static_cast<unsigned long>(hr));
        return false;
    }
...
bool InstallD3D8Hooks() {
    ...
    if (!ok) {
        OVERLAY_LOG("failed to hook D3D8 EndScene/Reset");
        return false;
    }

    g_installed = true;
    OVERLAY_LOG("D3D8 present hooks installed (EndScene, Reset)");
    return true;
}

void RemoveD3D8Hooks() {
    if (!g_installed) return;
    g_renderer8.Shutdown();
    g_installed = false;
}
